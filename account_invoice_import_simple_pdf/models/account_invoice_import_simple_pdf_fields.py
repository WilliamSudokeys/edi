...
class AccountInvoiceImportSimplePdfFields(models.Model):
    ...
    def get_value_from_list(  # noqa: C901
        ...
        if self.extract_rule == "max":
            ...
            if len(data_list) < self.position:
                error_msg = _(
                    "Partner '{partner_name}' is configured with an extract rule "
                    "'{extract_rule}' with position {position} for field "
                    "'{field_name}' but the list of extracted valid data only "
                    "has {entries_count} entries.",
            ...
            if len(data_list) < self.position:
                error_msg = _(
                    "Partner '{partner_name}' is configured with an extract rule "
                    "'{extract_rule}' with position {position} for field "
                    "'{field_name}' but the list of extracted "
                    "valid data only has {entries_count} entries.",
                    partner_name=self.partner_id.display_name,
                    extract_rule=test_info["extract_rule_sel"][self.extract_rule],
                    position=self.position,
                    field_name=test_info["field_name_sel"][self.name],
    ...
    def _get_date(self, parsed_inv, raw_text, partner_config, test_info):
        ...
        if not date_format:
            raise UserError(
                _(
                    "No date format configured on partner '{partner_name}' "
                    "nor on the field '{field_name}'.",
        ...
        if not date_separator:
            raise UserError(
                _(
                    "No date separator configured on partner '{partner_name}' "
                    "nor on the field '{field_name}'.",
                    partner_name=partner_name,
                    field_name=field_name,
                )
